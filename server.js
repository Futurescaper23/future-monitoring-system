...
const WORLDTIDES_API_KEY = process.env.WORLDTIDES_API_KEY || "b4de6263-82de-41e0-9bcc-21ed0551f1b7";
const ADMIN_PASSWORD = process.env.ADMIN_PASSWORD || "FutureScaping";
const PADSTOW_LATITUDE = "50.5400";
const PADSTOW_LONGITUDE = "-4.9400";
...
async function handleUploadRequest(request, response) {
  try {
    ...
    const allowedFiles = new Set(["ortho.jpg", "dsm.png", "contour.png", "section_lines.png", "section_profiles.csv"]);
...
async function handleVolumeChangeRequest(request, response) {
  try {
    ...
    const cleanedPolygons = polygons.map((item, index) => ({
      ...
      label: String(item.label || `Sandbar ${index + 1}`).trim(),
      gainM3: Number(item.gainM3 || 0),
      lossM3: Number(item.lossM3 || 0),
      netM3: Number(item.netM3 || 0),
      confidence: String(item.confidence || "Pending review").trim(),
      summary: String(item.summary || "No summary added yet.").trim()
    }));
...
function writeManifest(manifestPath, projectId, surveyId, areaId, areaDir) {
  const expectedFiles = ["ortho.jpg", "dsm.png", "contour.png", "section_lines.png", "section_profiles.csv"];
...
async function handleTideRequest(url, response) {
  try {
    ...
    upstream.searchParams.set("lat", PADSTOW_LATITUDE);
    upstream.searchParams.set("lon", PADSTOW_LONGITUDE);
    upstream.searchParams.set("date", startDate);
    upstream.searchParams.set("days", String(days));
    upstream.searchParams.set("datum", "CD");
    ...
    sendJson(response, 200, {
      items,
      extremes,
      datum: data.datum || data.responseDatum || "CD",
