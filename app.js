...
async function bootstrap() {
  ...
  state.adminMode = new URLSearchParams(window.location.search).get("admin") === "1" || window.localStorage.getItem("fsm-admin-mode") === "1";
  state.projectId = state.dataset.projects[0].id;
  state.surveyId = currentProject().surveys[0].id;
...
async function renderSections() {
  ...
  els.sectionDetails.innerHTML = [
    assetCard(
      ...
      `${area.summary} ${section.label} is a fixed cut through the area, so it helps us see how the ground rises, falls, and changes along one consistent line.`
    ),
    assetCard(
      "Current reading",
      hoverRow
    ...
    assetCard(
      "Survey context",
      `${survey.label} captured this area around low tide at ${area.lowTide}. ${activeProfiles.length > 1 ? "Use the survey profile toggles above the chart to compare rounds along the same fixed line. " : ""}${area.surveyNotes}`
    )
  ].join("");
  drawSectionChart(activeProfiles, section, survey.id);
  renderSectionComparisonSummary(activeProfiles, survey.id);
...
function renderWeather() {
  ...
  const params = new URLSearchParams({
    location: project.site?.name || "Padstow Harbour / Camel Estuary",
...
async function renderVolumeLegacy() {
  ...
  els.volumeNarrative.innerHTML = polygons.length
    ? [
      ...
      detail("Confidence", polygons.map((item) => `${item.label}: ${item.confidence || "Pending review"}`).join(" | "))
    ].join("")
    : [
    ...
    ? polygons.map((item) => `
      <article class="card">
        <div class="volume-card__meta">
          ...
          <span class="chip">${escapeHtml(item.confidence || "Pending review")}</span>
        </div>
...
async function renderVolume() {
  const project = currentProject();
  const sandboxComparisonEntry = Object.entries(project.volumeChangeComparisons || {}).find(([, entry]) => entry?.areas?.["area4"]);
  const sandboxArea = areas().find((item) => item.id === "area4") || currentArea();
    ...
    ? `
      <div class="volume-explainer">
        ...
        <div class="volume-explainer__block">
          ...
          <p>${escapeHtml(polygons.map((item) => `${item.label}: ${item.confidence || "Pending review"}`).join(" | "))}</p>
        </div>
      </div>
    `
    : `
    ...
    ? polygons.map((item) => `
      <article class="card volume-breakdown-card">
        <div class="volume-card__meta">
          ...
          <span class="chip volume-confidence-chip">${escapeHtml(item.confidence || "Pending review")}</span>
        </div>
        <div class="volume-card__grid">
          <div class="volume-mini volume-mini--gain">
...
async function renderAdmin() {
  ...
  const expectedFiles = [
    { id: "uploadOrtho", label: "ortho.png or ortho.jpg", description: "Aerial view export for the selected survey and area." },
    { id: "uploadDsm", label: "dsm.png", description: "Colour elevation export for the selected survey and area." },
    { id: "uploadContour", label: "contour.png or contours.png", description: "Contour overlay for the selected survey and area." }
  ];
  ...
  els.volumeMethodInput.value = volumeDataset?.method || project.volumeChangeComparisons?.[survey.id]?.method || "DSM difference raster clipped to fixed sandbar polygons";
...
function drawChart(profiles) {
  ...
  function renderSectionMap(area, section, basemapPath, overlayPath, basemapExists, overlayExists, rows) {
    ...
    els.sectionHotspots.innerHTML = area.sections.map((item) => `
      <button
        class="section-hotspot ${item.id === section.id ? "active" : ""}"
        type="button"
        data-section-hotspot="${escapeHtml(item.id)}"
        data-hotspot-x="${fixed(item.hotspot.x, 4)}"
        data-hotspot-y="${fixed(item.hotspot.y, 4)}"
      >
  ...
  els.sectionHotspots.querySelectorAll("[data-section-hotspot]").forEach((button) => {
    button.addEventListener("click", () => {
    ...
    els.sectionMapMarker.classList.toggle("hidden", !basemapExists && !overlayExists);
    requestAnimationFrame(() => positionSectionHotspots());
    updateSectionMapMarker(rows, section, state.sectionHoverDistance);
  }
  ...
  function sectionImageFrame() {
    ...
    const stage = els.sectionHotspots;
    if (!image || !stage) {
      return null;
    }
...
function drawSectionChart(profiles, section, anchorSurveyId = state.surveyId) {
  ...
  els.profileChart.onmousemove = (event) => {
    ...
    if (state.sectionHoverDistance === null || Math.abs(state.sectionHoverDistance - nextHoverDistance) > 0.5) {
...
function updateSectionMapMarker(rows, section, hoverDistance = state.sectionHoverDistance) {
  ...
  const markerPoint = hoverRow ? sectionPointForRow(section, hoverRow, rows) : section.hotspot;
...
function areas() {
  return Array.from({ length: 8 }, (_, index) => buildArea(index + 1));
}
...
function imageryLayers(area) {
  ...
  function buildArea(number) {
    ...
    const sectionTracks = explicitTracks?.length
      ...
      : [
        { start: { x: 48.2, y: 17.1 }, end: { x: 38.9, y: 17.1 } },
        { start: { x: 51.1, y: 66.8 }, end: { x: 28.8, y: 66.8 } },
        { start: { x: 69.3, y: 86.2 }, end: { x: 52.6, y: 93.4 } }
      ];
  return {
    id: key,
    overviewCode: `A${number}`,
    day: overview?.day || "Day 1",
    ...
    zone: overview?.zone || "Monitoring area",
    filterKey: overview?.filterKey || "all",
    statusLabel: overview?.statusLabel || "Monitoring area",
    ...
    missionRole: overview?.missionRole || "Monitoring area within the shared estuary programme.",
    operationalNote: overview?.operationalNote || "Operational notes will be added as survey rounds are documented.",
    deliverables: overview?.deliverables || "High-resolution aerial image, surface height model, 3D ground model, future comparison views",
    ...
    cardNote: overview?.cardNote || "Monitoring area",
    tags: overview?.tags || ["Ortho", "DSM", "Contours", "Sections"],
    statusNote: "Prototype assets are available. Future survey rounds should attach new outputs to this same area key.",
    layers: {
      ortho: {
        label: "Aerial View",
        fileType: "PNG / JPG",
        description: `Base aerial imagery for ${areaLabel}.`,
        fileName: "ortho.jpg"
      },
      dsm: {
        label: "Colour Elevation",
        fileType: "PNG",
        description: `Colour elevation map for ${areaLabel}.`,
        fileName: "dsm.png"
      },
      contours: {
        label: "Contours",
        fileType: "PNG",
        description: `Contour overlay for ${areaLabel}.`,
        fileName: "contour.png"
      },
      sections: {
        label: "Section Lines",
        fileType: "PNG",
        description: `Section-line overlay for ${areaLabel}.`,
        fileName: "section_lines.png"
      }
    },
    sections: [1, 2, 3].map((section, index) => {
...
function weatherWindowForSurvey(surveys, surveyId) {
  ...
  const end = current?.dateTo || current?.dateFrom || "2026-03-23";
  return {
    start: subtractOneMonth(end),
    end
  };
}
  ...
  return String(text || "")
    ...
    .map((line, index) => {
      ...
      return {
        ...
        label: label || `Sandbar ${index + 1}`,
        gainM3: Number.parseFloat(gain || "0") || 0,
        lossM3: Number.parseFloat(loss || "0") || 0,
        netM3: Number.parseFloat(net || "0") || 0,
        confidence: confidence || "Pending review",
        summary: summary || "No summary added yet."
      };
    });
}
...
function surveyAssetPath(projectId, surveyId, areaId, fileName) {
  ...
  return `./survey-data/${projectId}/${folder}/${areaId}/${fileName}`;
}

function sharedAreaAssetPath(projectId, areaId, fileName) {
  return `./shared-data/${projectId}/${areaId}/${fileName}`;
}

  function assetFileNameVariants(areaId, fileName) {
    const variants = [fileName];
    if (fileName === "ortho.jpg") {
      variants.unshift("ortho.png");
    }
    if (fileName === "contour.png") {
      variants.unshift("contours.png");
    }
  if (fileName === "section_lines.png") {
    variants.unshift("section-lines.png");
  }
    if (fileName === "section_profiles.csv") {
      variants.unshift(`${areaId}_section_profiles.csv`);
      if (areaId === "area7") {
        // Area 7's shared CSV was exported with an Area 8 filename and is currently file-locked in OneDrive.
        variants.unshift("area8_section_profiles.csv");
      }
    }
...
async function loadSharedAreaPolygons(projectId, areaId) {
  ...
  return features.map((feature, index) => {
    ...
    return {
      ...
      label: feature?.properties?.label || `Sandbar ${index + 1}`,
...
async function uploadSelectedFiles() {
  const uploads = [
    { input: els.uploadOrtho, fileName: "ortho.jpg" },
    { input: els.uploadDsm, fileName: "dsm.png" },
    { input: els.uploadContour, fileName: "contour.png" },
    { input: els.uploadSectionLines, fileName: "section_lines.png" },
    { input: els.uploadSectionCsv, fileName: "section_profiles.csv" }
  ];
...
function activateTab(tabName) {
  const safeTab = !state.adminMode && tabName === "admin"
    ? "overview"
    : tabName;
...
function syncAdminVisibility() {
  document.querySelectorAll("[data-admin-only='true']").forEach((item) => {
    item.classList.toggle("hidden", !state.adminMode);
  });
  els.adminModeToggle.classList.toggle("hidden", !isLocalHost() && !state.adminMode);
  els.adminModeToggle.textContent = state.adminMode ? "Close Admin Console" : "Open Admin Console";
  if (!state.adminMode && state.activeTab === "admin") {
